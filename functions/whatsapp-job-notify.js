...
exports.handler = async function (event, context, callback) {
    try {
        ...
        if (jobs.length > 0) {
            sendMessage(createMessage(jobs));
        }
        callback(null, {
            statusCode: 200,
            body: 'Success'
...
const getLatestJobs = async () => {
    ...
    const jobsPostedToday = jobs.filter(job => job.posted_date && job.posted_date.includes("hour"));
...
function parseData(html) {
    ...
    for (let i = 0; i < nodes.length; i++) {
        ...
        let job = {
            ...
            posted_date: $(element).find('ul.meta>li.date')
                .text()
